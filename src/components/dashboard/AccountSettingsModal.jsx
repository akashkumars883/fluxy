      ...
      {isOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-2 sm:p-4 animate-in fade-in duration-300">
          ...
          <motion.div 
            ...
            className="relative bg-white w-full max-w-lg rounded-[24px] shadow-2xl border border-zinc-200/80 flex flex-col max-h-[90vh] overflow-hidden z-10"
          >
            {/* Header */}
            ...
            <div className="p-6 space-y-6 overflow-y-auto flex-1 min-h-0 no-scrollbar">
              {/* Profile Section */}
              <div className="flex flex-col sm:flex-row items-center gap-5 bg-zinc-50/50 p-5 rounded-[20px] border border-zinc-100">
                <div className="relative group shrink-0">
                  {avatarUrl ? (
                    <img src={avatarUrl} alt={fullName} className="w-20 h-20 rounded-2xl object-cover border-4 border-white shadow-sm" />
                  ) : (
                    <div className="w-20 h-20 bg-zinc-100 text-zinc-400 rounded-2xl flex items-center justify-center border-4 border-white shadow-sm">
                      <User size={32} />
                    </div>
                  )}
                  <div className="absolute -bottom-2 -right-2 w-7 h-7 bg-zinc-950 text-white rounded-xl flex items-center justify-center border-2 border-white shadow-sm cursor-pointer hover:bg-zinc-800 transition-colors">
                    <Camera size={12} />
                  </div>
                </div>
                ...
                <div className="text-center sm:text-left flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-center sm:justify-start gap-2 mt-1.5 flex-wrap">
                    <span className="px-2 py-0.5 bg-[#6366F1]/10 text-[#6366F1] text-[10px] font-bold uppercase tracking-wider rounded-lg border border-[#6366F1]/20">
                      {provider} Verified
                    </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="p-4 bg-white rounded-2xl border border-zinc-200/80 flex items-start gap-3 shadow-sm">
                  <div className="w-8 h-8 bg-zinc-50 rounded-lg flex items-center justify-center text-zinc-500 border border-zinc-200/60 shrink-0">
                    <Mail size={14} />
                  </div>
              ...
              {/* Footer Info */}
              <div className="p-5 bg-indigo-50/50 rounded-[20px] border border-indigo-100/50 flex items-start gap-3.5">
                <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-[#6366F1] shadow-sm border border-indigo-100 shrink-0">
                  <Calendar size={14} />
                </div>
                <div>
            ...
            {/* Action Button */}
            <div className="px-6 py-5 bg-zinc-50/50 border-t border-zinc-100 flex justify-end gap-3 shrink-0 rounded-b-[24px]">
              <button 
                onClick={onClose}
