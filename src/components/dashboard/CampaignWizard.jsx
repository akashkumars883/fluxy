             ...
             <div className="space-y-6">
                ...
                <div className="space-y-4">
                   <textarea 
                     ...
                     className="w-full h-32 text-base font-semibold text-foreground placeholder:text-zinc-200 outline-none border-2 focus:border-foreground bg-zinc-50/50 p-4 rounded-[24px] tracking-normal transition-all resize-none"
                   />
                   
                   <div className={`p-4 rounded-[24px] border-2 transition-all ${hasLink ? 'bg-emerald-50 border-emerald-500/20' : 'bg-zinc-50/50 border-dashed border-border/40 opacity-50'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <MousePointer2 size={14} className="text-emerald-600" />
                        <span className="text-[9px] font-semibold text-emerald-700 tracking-normal">
                          {hasLink ? "Button Active" : "Add Link for button"}
                        </span>
                      </div>
                      <input 
                        type="text" 
                        placeholder={hasLink ? "Button Label" : "No link detected"}
                        value={buttonText}
                        disabled={!hasLink}
                        onChange={(e) => setButtonText(e.target.value)}
                        className="w-full bg-transparent border-b border-border/60 py-1 outline-none text-[11px] font-semibold tracking-normal focus:border-emerald-500"
                      />
                   </div>
                </div>
             </div>
