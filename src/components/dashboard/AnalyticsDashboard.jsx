  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 w-full max-w-[1400px] mx-auto pb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        <div>
          <p className="text-xs font-bold text-black opacity-60 uppercase tracking-widest mb-0.5">Overview</p>
          <h2 className="text-xl sm:text-2xl font-bold text-zinc-950 tracking-tight">Analytics</h2>
          <p className="text-sm text-black opacity-60 font-medium mt-0.5">Performance across all your automation campaigns.</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 text-[11px] font-bold px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-md border border-emerald-100">
        ...
        {metricCards.map((card, idx) => {
          ...
          return (
            <motion.div
              ...
              className="group relative bg-white border border-zinc-200/40 rounded-md p-4 flex flex-col gap-2.5 hover:border-zinc-300 hover: transition-all duration-200 cursor-default overflow-hidden (0,0,0,0.05)]"
            >
              {/* Hover gradient glow */}
              ...
              <div className="flex items-center justify-between">
                <div className={`w-9 h-9 ${card.softBg} rounded-md flex items-center justify-center group-hover:scale-105 transition-transform duration-200`}>
                ...
                {card.trend ? (
                  ...
                  <span className="text-[10px] font-semibold text-black opacity-60 bg-zinc-50 border border-zinc-100 px-2 py-1 rounded-lg">{card.desc}</span>
                )}
              </div>
              <div>
                <div className="text-2xl font-black text-black tracking-tight leading-none">{card.value}</div>
                <div className="text-[10px] font-bold text-black opacity-60 mt-1 uppercase tracking-wider">{card.label}</div>
              </div>
            </motion.div>
          );
        })}
        ...
        <div className="relative overflow-hidden bg-zinc-950 rounded-md p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              ...
              <p className="text-[11px] text-black opacity-60 mt-0.5 max-w-md font-medium leading-relaxed">
                {currentPlan === "free"
          ...
          <button
            ...
            className="shrink-0 inline-flex items-center gap-2 px-5 py-2.5 bg-white hover:bg-zinc-50 text-zinc-950 text-xs font-bold rounded-md transition-all duration-200 hover: hover:scale-[1.02] cursor-pointer relative z-10"
          >
      ...
      <div className="bg-white border border-zinc-200/60 rounded-md overflow-hidden ">
        <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-zinc-100/80">
          <div>
            ...
            <p className="text-[11px] text-black opacity-60 font-medium mt-0.5 ml-8">Performance per automation rule.</p>
          </div>
          <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1.5 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-md">
        ...
        {campaigns.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center gap-4">
            ...
            <div>
              <p className="text-sm font-bold text-black opacity-90">No campaign data yet</p>
              <p className="text-xs text-black opacity-60 mt-0.5">Create automations to start tracking performance.</p>
            </div>
          </div>
        ) : (
          <>
              ...
              <thead>
                <tr className="bg-zinc-50/80 border-b border-zinc-100">
                  {["Campaign", "Keyword", "Messages Sent", "Contacts", "Conv. Rate"].map(h => (
                    <th key={h} className="px-6 py-3.5 text-[9px] font-bold text-black opacity-60 uppercase tracking-widest">{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={i} className="hover:bg-zinc-50/60 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-semibold text-black truncate max-w-[160px]">{c.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-black">{c.sentFmt}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                ...
                <div key={i} className="px-5 py-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs font-bold text-black truncate max-w-[160px]">{c.name}</span>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={label} className="bg-zinc-50 rounded-md p-2.5">
                        <p className="text-[9px] text-black opacity-60 font-bold uppercase tracking-wider mb-1">{label}</p>
                        <p className={`text-sm font-black ${green ? "text-emerald-600" : "text-black"}`}>{val}</p>
                      </div>
                    ))}
                  </div>
                </div>
