...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (authUser) {
        let query = supabase.from("automations").select("*").eq("user_id", authUser.id);
        let { data: accountsRaw } = await query;

        if (accountsRaw) {
          loadedAccounts = accountsRaw;
        }
      } else {
        // Local-only dev bypass for testing connected accounts.
        let query = supabase.from("automations").select("*").eq("user_id", "dev-bypass");
        let { data: accountsRaw } = await query;
      ...
      <div className="flex flex-1 relative overflow-hidden">
        ...
        <main className="flex-1 p-6 sm:p-10 max-w-6xl mx-auto w-full overflow-y-auto">
          ...
          {isNoAccounts ? (
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {/* Template 1 */}
                  <div className="bg-white border border-zinc-200/80 rounded-[32px] p-6 flex flex-col justify-between hover:shadow-xl transition-all group hover:border-sage/30">
                    <div>
                      <div className="w-12 h-12 bg-purple-50 text-purple-600 border border-purple-100 rounded-2xl flex items-center justify-center mb-5 group-hover:scale-105 transition-all">
                        <MessageSquare size={22} />
                      </div>
                      <h3 className="text-lg font-bold text-zinc-900 mb-2 leading-snug">Comment-to-DM AutoReply</h3>
                      <p className="text-xs text-zinc-400 font-semibold leading-relaxed mb-6">
                        Instantly send a custom DM containing your signup or checkout link when a user comments a specific keyword on your post.
                      </p>
                    </div>
                    <button 
                      onClick={handleConnectClick}
                  ...
                  <div className="bg-white border border-zinc-200/80 rounded-[32px] p-6 flex flex-col justify-between hover:shadow-xl transition-all group hover:border-sage/30">
                    <div>
                      <div className="w-12 h-12 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-2xl flex items-center justify-center mb-5 group-hover:scale-105 transition-all">
                        <Sparkles size={22} />
                      </div>
                      <h3 className="text-lg font-bold text-zinc-900 mb-2 leading-snug">Story Mention Responder</h3>
                      <p className="text-xs text-zinc-400 font-semibold leading-relaxed mb-6">
                        Boost your brand loyalty by instantly replying with a personalized DM whenever a follower mentions your account in their story.
                      </p>
                    </div>
                    <button 
                      onClick={handleConnectClick}
                  ...
                  <div className="bg-white border border-zinc-200/80 rounded-[32px] p-6 flex flex-col justify-between hover:shadow-xl transition-all group hover:border-sage/30">
                    <div>
                      <div className="w-12 h-12 bg-blue-50 text-blue-600 border border-blue-100 rounded-2xl flex items-center justify-center mb-5 group-hover:scale-105 transition-all">
                        <Cpu size={22} />
                      </div>
                      <h3 className="text-lg font-bold text-zinc-900 mb-2 leading-snug">24/7 FAQ Chatbot</h3>
                      <p className="text-xs text-zinc-400 font-semibold leading-relaxed mb-6">
                        Instantly resolve typical user questions regarding pricing, shipping, or hours directly in Instagram DMs without lifting a finger.
                      </p>
                    </div>
                    <button 
                      onClick={handleConnectClick}
            ...
            /* --- STANDARD LOGGED STATE (IF ACCOUNTS PRESENT) --- */
            <div className="space-y-10">
